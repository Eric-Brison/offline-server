...
class offFolderListFormat
{
    ...
    static public function getSyncStatus(Doc & $doc)
    {
        ...
        if (count($domains) > 0) {
            ...
            global $action;
            $img = sprintf('<img class="icon" src="%s"> ', $action->parent->getImageLink('domainsync.png', true, 20));
        ...
        if ($w->select(array(
            ...
            switch ($w->status) {
                case docWait::invalid:
                    $img = sprintf('<img title="%s" class="icon" src="%s"> ', _("off:invalid") , $action->parent->getImageLink('status_invalid.png', true, 20));
                    $message = $w->statusmessage;
                    break;

                case docWait::constraint:
                    $img = sprintf('<img title="%s" class="icon" src="%s"> ', _("off:constraint") , $action->parent->getImageLink('status_constraint.png', true, 20));
                    $message = $w->statusmessage;
                    break;

                case docWait::upToDate:
                    $img = sprintf('<img title="%s" class="icon" src="%s"> ', _("off:upToDate") , $action->parent->getImageLink('status_uptodate.png', true, 20));
                    $message = sprintf("last update %s", FrenchDateToLocaleDate($w->date));
                    break;

                case docWait::conflict:
                    $img = sprintf('<img title="%s" class="icon" src="%s"> ', _("off:conflict") , $action->parent->getImageLink('status_conflict.png', true, 20));
                    $message = $w->statusmessage;
                    break;
