...
function off_update(&$action) {
    ...
    switch( $parms['download'] ) {
        case 'update':
            return off_update_download_update($parms, $action);
            break;
        case 'complete':
            return off_update_download_complete($parms, $action);
            break;
        case 'partial':
            return off_update_download_partial($parms, $action);
            break;
    }
    return off_update_unknown_download($parms, $action);
}
...
function off_update_download_update(&$parms, &$action) {
    ...
    $xml .= sprintf("<update type=\"major\" version=\"%s\" extensionVersion=\"%s\" buildID=\"%s\" detailsURL=\"%s\" >\n",
        ...
        $completeInfo['version'],
        $completeInfo['buildid'],
        sprintf("%s?app=OFFLINE&amp;action=OFF_DLCLIENT", $core_externurl)
    );
    ...
    if( $parms['version'] == $partialInfo['version_from'] && $parms['buildid'] == $partialInfo['buildid_from'] ) {
        ...
        $xml .= sprintf("  <patch type=\"partial\" URL=\"%s\" hashFunction=\"%s\" hashValue=\"%s\" size=\"%s\" />\n",
            sprintf("%sguest.php?app=OFFLINE&amp;action=OFF_UPDATE&amp;download=partial&amp;version=%%VERSION%%&amp;buildid=%%BUILD_ID%%&amp;os=%s&amp;arch=%s",
                $core_externurl,
                $parms['os'],
                $parms['arch']
            ),
            $partialInfo['hashfunction'],
            $partialInfo['hashvalue'],
            $partialInfo['size']
        );
    }
    ...
    $xml .= sprintf("  <patch type=\"complete\" URL=\"%s\" hashFunction=\"%s\" hashValue=\"%s\" size=\"%s\" />\n",
        sprintf("%sguest.php?app=OFFLINE&amp;action=OFF_UPDATE&amp;download=complete&amp;version=%%VERSION%%&amp;buildid=%%BUILD_ID%%&amp;os=%s&amp;arch=%s",
            $core_externurl,
            $parms['os'],
            $parms['arch']
        ),
        $completeInfo['hashfunction'],
        $completeInfo['hashvalue'],
        $completeInfo['size']
    );
